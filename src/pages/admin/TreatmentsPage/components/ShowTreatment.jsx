...
import { fetchTreatmentById } from "@/store/slices/treatmentSlice";

export default function ShowTreatment({ params }) {
  const { id } = useParams();
  ...
  // user selectors -------------
  const item = useSelector(selectSelectedTreatment);
  const loading = useSelector(selectTreatmentsLoading);
  const errorData = useSelector(selectTreatmentsError);
  // ----------------------------
  useEffect(() => {
    if (id) {
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Basic details and treatment information
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="email">Dosage</Label>
                  <p className="text-sm mt-1">{item.dosage ?? "non"}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm mt-1">{item.instructions ?? "non"}</p>
                </div>
                <div>
                  <Label htmlFor="department">Notes</Label>
                  <p className="text-sm mt-1">{item.notes ?? "non"}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <p className="text-sm mt-1">{item.quantity ?? 0}</p>
              </div>
              <div>
                <Label>Refills</Label>
                <p className="text-sm mt-1">{item.refills ?? 0}</p>
              </div>
              <Separator />
              <div>
