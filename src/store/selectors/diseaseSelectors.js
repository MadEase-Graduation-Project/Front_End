...
export const selectAllDiseases = state => selectDiseasesState(state).items;
export const selectDiseasesLoading = state => selectDiseasesState(state).loading;
export const selectDiseasesError = state => selectDiseasesState(state).error;
export const selectSelectedDisease = state => selectDiseasesState(state).selectedDisease;
...
export const selectFilteredDiseases = createSelector(
  [selectAllDiseases, (state, searchQuery) => searchQuery, (state, _, selectedCategory) => selectedCategory],
...
export const makeSelectDiseaseById = () => 
  createSelector(
    ...
    (diseases, diseaseId) => diseases.find(disease => disease._id === diseaseId)
  );
...
export const selectDiseasesByCategory = createSelector(
  ...
  (diseases, category) => {
    ...
    return diseases.filter(disease => disease.diseasesCategoryName === category);
  }
);
