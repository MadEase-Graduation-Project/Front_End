...
export const selectAllDoctors = (state) => selectDoctorsState(state).doctors;
export const selectDoctorsLoading = (state) =>
  selectDoctorsState(state).loading;
export const selectDoctorsError = (state) => selectDoctorsState(state).error;
export const selectSelectedDoctor = (state) =>
  selectDoctorsState(state).doctorDetails;
export const selectPublicDoctors = (state) =>
  selectDoctorsState(state).publicDoctors;

//selectDoctorsPagination

export const selectDoctorsPagination = (state) => ({
  currentPage: selectDoctorsState(state).currentPage,
  totalPages: selectDoctorsState(state).totalPages,
  hasMore: selectDoctorsState(state).hasMore,
  loadingMore: selectDoctorsState(state).loadingMore,
});
...
//     return [
//       "All Specialties",
//       ...Array.from(new Set(doctors.map((doctor) => doctor.specialty))).filter(
//         Boolean
//       ),
//     ];
//   }
...
//     (state, _, selectedSpecialty) => selectedSpecialty,
//   ],
...
//     return doctors.filter((doctor) => doctorIdsWithAdvices.has(doctor._id));
//   }
// );
